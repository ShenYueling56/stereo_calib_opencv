...
//    ofstream stereoStore(stereoCalibrateResult);
    cv::FileStorage stereoStore(stereoCalibrateResult, cv::FileStorage::WRITE);
    ...
    stereoStore << "distCoeffsR" << distCoeffs2;
    stereoStore << "R" << R;
    stereoStore << "T" << T;
    stereoStore << "E" << E;
    stereoStore << "F" << F;
...
{
    cv::CommandLineParser parser(argc, argv, "{w|11|}{h|8|}{s|25|}{d|/home/shenyl/Documents/sweeper/data/|}{show|False|}{help||}");
    if (parser.has("help"))
    {
        parser.printMessage();
        return 0;
